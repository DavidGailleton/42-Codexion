...
unsigned int	get_process_time(t_config *config)
{
	...
	now_ms = (unsigned int)time.tv_sec * 1000 + (unsigned int)time.tv_usec
		/ 1000;
...
unsigned int	get_remain_before_burnout(t_config *config, t_coder *coder)
{
	unsigned int	now_ms;
	unsigned int	last_compile_ms;
...
struct timespec	abs_time_burnout(t_config *config, t_coder *coder)
{
	...
	nsec = (long long)config->programm_start_time.tv_usec * 1000LL + (total_ms
			% 1000) * 1000000LL;
